...
- The API response is cast to the TypeScript types, not validated at runtime. With a known mock server this is proportionate; a production version would validate with something like Zod at the boundary.
- Stored labels are a snapshot from selection time. If a form were renamed mid-session the badge would show the old name. The ids stay correct, and the graph is fetched once per session, so this does not bite here.
- Mappings live in memory and reset on reload. Persistence (localStorage or a backend) would slot into the reducer layer without touching the UI.
- All rows re-render when any mapping changes, since they read one state context. At this scale it is irrelevant; per-field selector contexts would be the fix if it ever mattered.
